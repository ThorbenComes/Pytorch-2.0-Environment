...
from utils.functions import elup1 as elup1, elup1_inv as elup1_inv

nn = torch.nn


@torch.compile
def invert(S=Iterable[torch.Tensor]):  # checked
    """
    ...
    def _masked_update(self, prior_mean: torch.Tensor, prior_cov: Iterable[torch.Tensor],
        ...
        if self._mem:
            if obs_mean.shape[1] == 1:
                ...
                prior_lam_u, prior_lam_l, prior_lam_s = invert(
                ...
                [post_cov_u, post_cov_l, post_cov_s] = invert(
...
class ProcessNoise(nn.Module):
    def __init__(self, lsd, init_trans_covar, num_hidden: Iterable[int], activation: str,
        ...
        self._log_process_noise = nn.Parameter(
            nn.init.constant_(torch.empty(1, self._lsd, dtype=self._dtype), init_trans_cov))
...
class Predict(nn.Module):
    ...
    def __init__(self, latent_obs_dim: int, act_conditioned: bool = True, hierarchy_type: str = None, config=None,
                 dtype: torch.dtype = torch.float32):
        """
        TODO: add references to block diagram
        RKN Cell (mostly) as described in the original RKN paper
        :param latent_obs_dim: latent observation dimension
        :param act_conditioned=: if action conditional
        ...
        self._lod = latent_obs_dim
        self._lsd = 2 * self._lod
        self._action_conditioned = act_conditioned
        ...
        assert self._hier_type in ["manager", "submanager", "worker", "worker_v2", "ACRKN", "HIPRSSM", "worker_wa"], \
        ...
        if self._action_conditioned:
            ...
            if self._hier_type in ["worker", "ACRKN", "HIPRSSM"]:
                ## control neural net
                self._b = Control(self._action_conditioned, self._lsd, self.c.control_net_hidden_units,
        ...
        if self._hier_type is "submanager" or self._hier_type is "worker" or self._hier_type is "worker_v2" or self._hier_type is "HIPRSSM" \
                or self._hier_type is "worker_wa":
    ...
    def forward(self, post_mean_list: Iterable[torch.Tensor], post_cov_list: Iterable[torch.Tensor]) -> \
        ...
        if self._hier_type == "manager":
            ...
            if self._action_conditioned:
                ...
                prior_cov_1 = [torch.zeros_like(post_cov_list[0][0]), torch.zeros_like(post_cov_list[0][1]),
                               torch.zeros_like(post_cov_list[0][2])]
        ...
        elif self._hier_type == "worker_wa":
            ## Worker with encoded action
            prior_mean_0, prior_cov_0 = gaussian_linear_transform(self._A, post_mean_list[0], post_cov_list[0])
            if self._action_conditioned:
                prior_mean_1 = post_mean_list[1]
            else:
                prior_mean_1 = torch.zeros_like(post_mean_list[0])
            prior_mean_2, prior_cov_2 = gaussian_linear_transform(self._C, post_mean_list[-1], post_cov_list[-1])
